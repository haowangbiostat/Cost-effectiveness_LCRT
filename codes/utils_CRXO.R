############################################################################################################
#                                                                                                          #
#        program:        utils_CRXO.R                                                                      #
#                                                                                                          #
#        usage:          LOD and MMD in multiple-period CRXOs for cost-effectiveness                       #
#                                                                                                          #
...
# compute theta parameter for optimal design
compute_vartheta_CRXO <- function(rho0_E, rho1_E, rho0_C, rho1_C, rho0_EC, rho1_EC, rho2_EC, lambda, r) {
  # numerator: related to within-cluster variance
  num <- (1 - rho1_E) + 2 * (rho1_EC - rho2_EC) / (lambda * r) + (1 - rho1_C) / (lambda^2 * r^2)
  
  # denominator: related to between-cluster variance  
  denom <- (rho0_E - rho1_E) + 2 * (rho1_EC - rho0_EC) / (lambda * r) + (rho0_C - rho1_C) / (lambda^2 * r^2)
...
# compute variance of INMB estimator (equation 3)
compute_variance_CRXO <- function(I, K, rho0_E, rho1_E, rho0_C, rho1_C, 
                                  rho0_EC, rho1_EC, rho2_EC, J, lambda, sigma_E, sigma_C, pi) {
  # rho0_E >= rho1_E
  ...
  var_beta <- (kappa_C * sigma_C^2 - 2 * lambda * kappa_EC * sigma_C * sigma_E + 
                 lambda^2 * kappa_E * sigma_E^2) / (I * J * K * pi * (1 - pi))
...
create_constraint_function_CRXO <- function(K, J) {
  function(x) {
    rho0_E <- x[1]
    rho1_E <- x[2]
    rho0_C <- x[3]
...
compute_LOD_integer_CRXO <- function(rho0_E, rho1_E, rho0_C, rho1_C, rho0_EC, rho1_EC, rho2_EC, 
  ...
  for (I in seq(2, max_I, by = piden)) {
    ...
    while (actcost <= B && K <= max_K) {
      var <- compute_variance_CRXO(I, K, rho0_E, rho1_E, rho0_C, rho1_C, 
...
compute_LOD_decimal_CRXO <- function(rho0_E, rho1_E, rho0_C, rho1_C, rho0_EC, rho1_EC, rho2_EC,
                                     trteff, zalpha, J, lambda, sigma_E, sigma_C, prop,
                                     c1, c2, B, r) {
  # compute vartheta
  vartheta <- compute_vartheta_CRXO(rho0_E, rho1_E, rho0_C, rho1_C, rho0_EC, rho1_EC, rho2_EC, lambda, r)
  ...
  var_dec <- compute_variance_CRXO(I_dec, K_dec, rho0_E, rho1_E, rho0_C, rho1_C,
                                   rho0_EC, rho1_EC, rho2_EC, J, lambda, sigma_E, sigma_C, prop)
...
create_mmd_objective_CRXO <- function(temp_I, temp_K, J, lambda, sigma_E, sigma_C, prop, c1, c2, B) {
  r <- sigma_E / sigma_C
  ...
  function(x) {
    ...
    var_dec <- compute_variance_CRXO(I_dec, K_dec, rho0_E, rho1_E, rho0_C, rho1_C,
                                     rho0_EC, rho1_EC, rho2_EC, J, lambda, sigma_E, sigma_C, prop)
    var_int <- compute_variance_CRXO(temp_I, temp_K, rho0_E, rho1_E, rho0_C, rho1_C,
                                     rho0_EC, rho1_EC, rho2_EC, J, lambda, sigma_E, sigma_C, prop)
...
compute_MMD_decimal_CRXO <- function(rho0_E_min, rho1_E_min, rho0_C_min, rho1_C_min, 
  ...
  vartheta_min <- compute_vartheta_CRXO(rho0_E_max, rho1_E_min, rho0_C_max, rho1_C_min,
                                        rho0_EC_max, rho1_EC_min, rho2_EC_max, lambda, r)
  
  vartheta_max <- compute_vartheta_CRXO(rho0_E_min, rho1_E_max, rho0_C_min, rho1_C_max,
                                        rho0_EC_min, rho1_EC_max, rho2_EC_min, lambda, r)
  ...
  if (rho0_E_min > rho1_E_min && rho0_C_min > rho1_C_min && rho0_EC_min > rho1_EC_min &&
    ...
    # decimal estimates for reference
    g_theta_min <- (sqrt(c2 * J * vartheta_min) + sqrt(c1))^2
    g_theta_max <- (sqrt(c2 * J * vartheta_max) + sqrt(c1))^2
...
compute_MMD_integer_CRXO <- function(rho0_E_min, rho1_E_min, rho0_C_min, rho1_C_min,
                                     rho0_EC_min, rho1_EC_min, rho2_EC_min,
                                     rho0_E_max, rho1_E_max, rho0_C_max, rho1_C_max,
                                     rho0_EC_max, rho1_EC_max, rho2_EC_max,
  ...
  for (I in seq(2, max_I, by = piden)) {
    ...
    while (actcost <= B && K <= max_K) {
      ...
      obj_fn <- create_mmd_objective_CRXO(temp_I, temp_K, J, lambda, sigma_E, sigma_C, prop, c1, c2, B)
      
      # create constraint function
      eval_g_ineq <- create_constraint_function_CRXO(K, J)
...
# ============================================================================================================
# main function for optimal design in cost-effectiveness CRXO trials
# ============================================================================================================

OD_CE_CRXO_MPD <- function(
  ...
  if (is_LOD) {
    ...
    lod_decimal <- compute_LOD_decimal_CRXO(
    ...
    lod_integer <- compute_LOD_integer_CRXO(
      rho0_E_min, rho1_E_min, rho0_C_min, rho1_C_min,
      rho0_EC_min, rho1_EC_min, rho2_EC_min,
      trteff, zalpha, J, lambda, sigma_E, sigma_C, prop,
      c1, c2, B, max_I, max_K, piden
    ...
    mmd_decimal <- compute_MMD_decimal_CRXO(
      rho0_E_min, rho1_E_min, rho0_C_min, rho1_C_min,
      rho0_EC_min, rho1_EC_min, rho2_EC_min,
      rho0_E_max, rho1_E_max, rho0_C_max, rho1_C_max,
      rho0_EC_max, rho1_EC_max, rho2_EC_max,
    ...
    mmd_integer <- compute_MMD_integer_CRXO(
      rho0_E_min, rho1_E_min, rho0_C_min, rho1_C_min,
      rho0_EC_min, rho1_EC_min, rho2_EC_min,
      rho0_E_max, rho1_E_max, rho0_C_max, rho1_C_max,
      rho0_EC_max, rho1_EC_max, rho2_EC_max,
